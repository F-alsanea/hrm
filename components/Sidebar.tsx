  ...
  return (
    <aside className="w-full md:w-80 glass m-2 md:m-4 rounded-[2rem] md:rounded-[3rem] overflow-hidden flex flex-col h-auto md:h-[calc(100vh-2rem)] shadow-2xl transition-all duration-300 z-30 border-2 border-white/30" style={{ backgroundColor: 'var(--bg-sidebar)' }}>
      <div className="p-6 md:p-10 text-center border-b border-white/10 bg-white/5">
        <div className="w-24 h-24 bg-white rounded-[2rem] mx-auto mb-6 flex items-center justify-center text-[#1f4e78] font-black text-4xl shadow-2xl rotate-3">
          K
        </div>
        <h2 className="font-black text-2xl text-white tracking-tight">مجموعة الكعكي</h2>
        <div className="mt-3 inline-block px-4 py-1.5 bg-white/10 rounded-full text-[11px] text-white/90 font-black tracking-widest border border-white/20">
      ...
      <nav className="p-4 md:p-8 space-y-2 md:space-y-4 flex-1 overflow-y-auto">
        {navItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 md:gap-5 px-4 md:px-6 py-3 md:py-5 rounded-[1.2rem] md:rounded-[1.8rem] transition-all duration-300 ${activeTab === item.id 
      ...
      <div className="p-4 md:p-8 space-y-4 md:space-y-6">
        <div className="bg-white/5 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-white/10 space-y-4 shadow-inner">
            ...
            <div className="flex bg-black/30 p-1 rounded-xl border border-white/5">
              {(['light', 'dark', 'sepia'] as ThemeMode[]).map(m => (
                <button
                  key={m}
                  onClick={() => setTheme(m)}
                  className={`flex-1 py-1.5 rounded-lg text-[9px] md:text-[10px] font-black tracking-tighter transition-all ${theme === m ? 'bg-white text-[#1f4e78] shadow-md' : 'text-white/40 hover:text-white/70'}`}
                >
                  {t[m]}
                </button>
              ))}
          ...
          <div className="flex flex-col gap-2">
            ...
            <button onClick={() => setLang(lang === 'ar' ? 'en' : 'ar')} className="bg-white/10 px-3 py-2 rounded-xl text-[10px] text-white font-black tracking-widest border border-white/10 transition-all uppercase">
              {lang === 'ar' ? 'Switch to English' : 'التبديل للعربية'}
            </button>
          </div>
        </div>

        <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 py-3 rounded-[1.2rem] bg-red-600/10 text-red-100 hover:bg-red-600 hover:text-white transition-all text-[10px] font-black uppercase tracking-widest border border-red-600/20 shadow-lg">
